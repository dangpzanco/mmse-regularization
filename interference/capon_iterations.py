...
def dlog_loss(beamformer, alpha):
    spec, norm_w, gamma = np.array(list(map(beamformer._iter_alpha, alpha))).T
    N = beamformer.N
    M = beamformer.M

    val = -gamma/alpha + N / (spec / (norm_w - 1/M) + alpha)
    return val
...
sinr_w = beamformer.sinr(w)
fig, ax = plt.subplots()
ax.plot(a, 10*np.log10(sinr_w))
ax.axvline(a_opt, color='k', linestyle='--')
for i, debug in enumerate(debug_list):
    ax.axvline(debug['mackay']['alpha'][-1], color=color_list[i], label=f'Mackay, $\\epsilon={eps_list[i]:.0e}$')
ax.axvline(debug['barber']['alpha'][-1], color=color_list[i+1], label='Barber')
ax.set_xscale('log')
